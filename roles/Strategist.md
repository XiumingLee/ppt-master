...
**在策略师开始工作之前，以下事项必须已完成**：

1. **项目文件夹已创建**：`projects/<项目名>_<格式>_<日期>/`
2. **模板选项已确认**（三选一）：
   - A) 使用已有模板 → 模板已复制到项目 `templates/` 目录
   - B) 不使用模板
   - C) 生成新模板 → 策略师完成大纲后调用 Template_Designer

**策略师需根据模板选项调整工作方式**：

- 如选择 A（使用已有模板）：需先查看模板文件，在八项确认中参考模板的配色、布局、风格
- 如选择 B/C：自由进行八项确认和设计规划
...
**选择 B/C/D 时**，在规范中添加「图片资源清单」，每张图片标注状态，如果是选择C `AI生成`图片，每一页不一定只有一张图，合理布局可以是多张小图。
...
以正文字号为基准 (1×)，通过比例推导其他层级：

- **14pt 基准**: 内容密集（咨询报告、数据分析）
- **18-20pt 基准**: 内容宽松（演讲、培训、展示）
...
> 💡 **px ↔ pt 转换**: 1 pt = 1.333 px（96 DPI）。SVG 用 px，PPTX 用 pt。
...
> **注意**：模板选项（A 使用已有 / B 不使用 / C 生成新模板）在流程启动时已确认，策略师根据该选项决定后续流程。
...
**下一步**: 调用 **Template_Designer** 角色生成页面模板。

模板将包含：
- 01_cover.svg      - 封面模板
- 02_chapter.svg    - 章节页模板
...
询问模板选项（流程步骤，非策略师职责）
    │
    ▼
Strategist (本角色) ← 八项确认 + 设计规范
    │
    ├─ 模板选项 = C（生成新模板）?
    │       │
    │       YES → Template_Designer → 项目 templates/
    │       │
    │       NO ─────────────────────────────────────┤
    │                                               │
    ├─ 图片方式包含「C) AI 生成」?                  │
    │       │                                       │
    │       YES → Image_Generator → images/         │
    │       │                                       │
    │       NO ─────────────────────────────────────┤
    │                                               │
    ▼                                               ▼
Executor（根据模板选项决定是否遵循模板）
```
