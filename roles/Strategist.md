...
```bash
# PPT 16:9 格式
python3 tools/project_manager.py init 季度业务报告 --format ppt169

# 小红书格式
python3 tools/project_manager.py init 产品推广 --format wechat
```

**执行后会创建**：

```
projects/<项目名>_<格式>_<YYYYMMDD>/
├── README.md
├── 设计规范与内容大纲.md（待保存）
├── svg_output/
└── images/（如需要）
